...
const addUser = async (args, transporter) => {
    ...
    const text = (
`Welcome to DSA2019, This email is just to inform you that your DSA Judge account is created.
Here are your account and password.

- Account: ${args.user}
- Password: ${randPass}

Head on to https://dsa2019.csie.org to try it !
` );

    const mailOptions = {
        from: '"DSA2019" <dsa1@csie.ntu.edu.tw>',
        to: args.user,
...
Head on to https://dsa2019.csie.org and change it.
` );

    const mailOptions = {
        from: '"DSA2019" <dsa1@csie.ntu.edu.tw>',
        to: args.user,
        subject: `[DSA2019]Password of your DSA Judge Account has been reset`,
        text,
    };
